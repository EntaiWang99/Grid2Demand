"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    ...
    Commit changes: 
    1) Based on 0418c, add total population and average trip rate 
    per person to control total trips. 
    The change is located at 1474-1491.
    2) Based on the function of PartitionGrid in 0604a, remove 
    centroids and zones with no demand.
    The changes are located at 581-582, 780-804.
    * The centroids will be overwritten when running several times.

    3) Entai: Distinguish the physcial zone and demand zone for each node; 
    dataframe output reformulation 2021/07/08
...
def ReadNetworkFiles(input_folder=None):
    ...
    with open(node_filepath, errors='ignore') as fp:
        ...
        for line in reader:
            try:
                is_centroid = line['activity_type']
...
def PartitionGrid(number_of_x_blocks=None,
    ...
    for block_no in range(1, block_numbers + 1):
        ...
        # for node in g_outside_boundary_node_list:
            if ((node.x_coord <= block.x_max) & (node.x_coord >= block.x_min) \
                ...
                node.phy_grid_id = str(block.id)
                
                # if (node.activity_type == 'poi' or node.activity_type == 'residential'):
        ...
        if len(block.node_id_list) != 0:
            ...
            if (len(block.node_id_list) != 0) and (block.poi_count == 0):
                block.zone_type = 'physcial_zone'
                g_phy_grid_list.append(block)
            if (block.poi_count != 0):
                block.zone_type = 'demand_zone'
    ...
    for block in g_zone_list:
        ...
        for node_id in block.node_id_list:
            if (link_id_temp < 50):
                if (g_node_list[g_node_id_to_index[node_id]].activity_location_tab in ['residential', 'boundary',
                                                                                       'poi']):
    ...
    #     line = ['name', 'link_id', 'osm_way_id', 'from_node_id', 'to_node_id', 'dir_flag', 'length', 'lanes',
    #             'free_speed', \
    ...
    if g_output_folder is not None:
        ...
        data['activity_zone_id'] = pd.DataFrame([poi.zone_id for poi in g_poi_list])
        data['area'] = pd.DataFrame([poi.area for poi in g_poi_list])
        # print(data)
    ...
    data_zone["grid_zone_id"] = pd.DataFrame([zone.id for zone in g_phy_grid_list])
    ...
    data_zone['shopping_poi_count'] = pd.DataFrame([zone.shopping_count for zone in g_zone_list])
    ...
    data_zone['zone_type'] =  pd.DataFrame([str(zone.zone_type) for zone in g_zone_list])
...
def GetNodeDemand(residential_production = None, residential_attraction = None,
    ...
    for node in g_node_list:
        ...
        elif node.activity_location_tab == 'boundary' and \
                (node.activity_type == 'primary' or node.activity_type == 'secondary' or
                 node.activity_type == 'motorway'):   # boundary node
    ...
    if g_output_folder is not None:
        ...
        data['phy_grid_id'] = pd.DataFrame([str(node.phy_grid_id) for node in g_node_list])
...
def RunGravityModel(population=None, AvgTripRatePerPerson=None, trip_purpose=None, a=None, b=None, c=None):
    ...
    # control total trips according to area population and average trip rate per person
    if population != None:
        if AvgTripRatePerPerson == None:
            number_of_trips_per_person = 2  # default value of average trip rate per person
            print("\nAverage number of daily trips per person is NOT defined. The default value is 2.")
            logger.info("\nAverage number of daily trips per person is NOT defined. The default value is 2.")
        else:
            number_of_trips_per_person = AvgTripRatePerPerson
        controlled_total_trips = population * number_of_trips_per_person
        adjustment_coefficient = controlled_total_trips/sum(volume_list)
        volume_list = [i * adjustment_coefficient for i in volume_list]
        print("\nThe OD demand is controlled by total population. The adjustment coefficient is " +
              str(round(adjustment_coefficient,4)))
        logger.info("\nThe OD demand is controlled by total population. The adjustment coefficient is " +
              str(round(adjustment_coefficient,4)))
    else:
        print("Total population of the area of interest is NOT given. The OD demand won't be adjusted.")
        logger.info("Total population of the area of interest is NOT given. The OD demand won't be adjusted.")
